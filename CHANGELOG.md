...
### Added
- 程式碼優化：**魔法數字抽離**。將 `GameEngine` 與 `EntityManager` 中所有魔法數字抽離為類別靜態常數（30+ 個常數），大幅提升程式碼可維護性與可讀性。
- 程式碼優化：**效能提升**。實作圖片預載入機制，遊戲開始前先載入所有圖片，避免第一隻怪物顯示延遲。將魔法數字抽離為 `GameEngine` 類別常數，提升程式碼可維護性。新增 `fontSize` 快取減少 `measureText` 重複計算。為 `monsterImage` 新增 `onerror` 錯誤處理。
- 新增文檔：**OPTIMIZATION.md**。記錄程式碼效能優化與功能改進建議。
- 新增功能：**最高紀錄**。遊戲結束時會自動儲存最高分數至瀏覽器 localStorage，並在首頁與遊戲結束畫面顯示「最高紀錄: X」。破紀錄時會顯示「NEW RECORD! / 新紀錄!」動畫效果。
- 新增功能：**遊戲模式選擇**。首頁新增三種運算模式供玩家選擇：「加法模式」（一位數加一位數，答案 0-18）、「減法模式」（被減數 1-18、減數 0-9，答案限非負一位數 0-9）、「乘法模式」（一位數乘一位數，答案 1-81）。
- 新增功能：**難度選擇**。首頁新增三種難度等級供玩家選擇：「困難」（400分過關）、「中等」（350分過關）、「簡單」（200分過關）。
